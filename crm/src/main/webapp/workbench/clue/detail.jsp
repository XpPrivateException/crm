		...
		<script type="text/javascript">
			...
			$(function(){
						...
						$.ajax({
							...
							success: function(data){
								...
								$.each(data,function(index,element){
									html += '<tr>';
									html += '<td><input type="checkbox" name="activityBox" onclick="activityCheck()" value="' + element.id + '"/></td>';
				...
				$("#bundBtn").click(function(){
					var $checkBox = $(":input[name=activityBox]:checked");
					if($checkBox.length == 0){
						//没选择
						alert("请选择需要关联的市场活动!");
					}else{
						//选择1或更多
						//data:.do?clueId=xxx&activityId=xxx&activityId=xxx&activityId=xxx
			...
			function showActivityList(){
				...
				$.ajax({
					...
					dataType: "json",
					success: function(data){
						//data: [{市场活动1},{市场活动1},{市场活动1}]，其中的id为关系表中对应的id
						var html = "";
						$.each(data,function(index,element){
							html += "<tr>";
							html += "<td>" + element.name + "</td>";
			...
			function unbund(relationId){
				//通过市场活动的id，和线索id(EL中)
				if(window.confirm("确定接触与该市场活动的关联吗?")) {
					$.ajax({
